...
// The new default has to be an address the client will actually accept, or
// every install lands on a relay its own guard refuses.
func TestNewDefaultPassesTheRelayGuard(t *testing.T) {
	if err := ValidateRelayURL(ownDomainRelay); err != nil {
		t.Errorf("the shipped default is refused by the client: %v", err)
	}
}
